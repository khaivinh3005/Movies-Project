...
import { loginFireBase, QuanLyNguoiDung } from '../../redux/actions/QuanLiNguoiDungAction';
import { TOKEN } from '../../redux/actions/types/CarouselTypes';
import { Redirect } from 'react-router-dom';
import { history } from '../../App';
import { signInWithGooglePopup ,createUserDocumentFormAuth } from '../../util/firebase/firebase';
...
export default function Login(props) {
    ...
    const formik = useFormik({
        ...
        onSubmit: (values) => {
            dispatch(QuanLyNguoiDung(values))
            setIsLoggedIn(true);  
        }
    });
    ...
    return (
        <section className="min-h-screen flex items-stretch text-white ">
            <div className="lg:flex w-1/2 hidden bg-gray-500 bg-no-repeat bg-cover relative items-center" style={{ backgroundImage: 'url(https://cdn.vietnambiz.vn/2020/2/14/ticketview-15816704441281296608294.jpg)' }}>
            ...
            <div className="lg:w-1/2 w-full flex items-center justify-center text-center md:px-16 px-0 z-0" style={{ backgroundColor: '#161616' }}>
                ...
                <div className="w-full py-6 z-20">
                    ...
                    <form action className="sm:w-2/3 w-full px-4 lg:px-0 mx-auto" onSubmit={formik.handleSubmit}>
                        <div className="pb-2 pt-4">
                            <input onChange={formik.handleChange}
                                onBlur={formik.handleBlur}
                                value={formik.values.taiKhoan} type="text" name="taiKhoan" placeholder="tai Khoan" className="block w-full p-4 text-lg rounded-sm bg-black" />
                        ...
                        <div className="pb-2 pt-4">
                            <input onChange={formik.handleChange}
                                onBlur={formik.handleBlur}
                                value={formik.values.matKhau} className="block w-full p-4 text-lg rounded-sm bg-black" type="text" name="matKhau" id="matKhau" placeholder="matKhau" />
                             {formik.touched.matKhau && formik.errors.matKhau ? (
                                <span className=" text-red-500 ">{formik.errors.matKhau}</span>
                            ) : null}    
                        </div>
                        <div className=" text-gray-400 hover:underline hover:text-gray-100">
                            <span className="cursor-pointer" onClick={logGoogleUser}>Sign In with google <GoogleOutlined className=" text-sm mb-1 fixIcon"/></span>
                        </div>
                        ...
                        <div className="px-4 pb-2 pt-4">
                            <button type="submit" className="uppercase block w-full p-4 rounded-full bg-indigo-500 hover:bg-indigo-600 focus:outline-none">sign in</button>
                        </div>
